...
namespace WebLight.Channel
{
    ...
    /// <remarks>
    ///     <para>
    ///         This channel will enqueue all outbound messages internally to be able to send them as effectively as possible.
    ///         There is therefore no guarantee that
    ///         the packets have reached the OS socket library just because the send method completes. If you want to be sure
    ///         that your message have been sent you need
    ...
    ///     </para>
    ///     <para>
    ///         Either by providing encoder/decoder (or one of them) which means that you can send/receive objects (they will
    ///         be encoded/decoded near to the actual IO operation).
    ///     </para>
    ///     <para>
    ...
    public class SocketChannel
    {
        ...
        /// <summary>
        /// If set, it will be used to encode all outbound messages (except <c>byte[]</c> and <c>BufferSlice</c>).
        /// </summary>
        ...
        /// <summary>
        ///     To do a graceful disconnect (not required if you have recieved <see cref="ChannelFailed" /> with an socket error or
        ///     with an <c>SocketException</c>)
        /// </summary>
        ...
        /// <summary>
        /// Start socket and begin listening for inbound data.
        /// </summary>
        /// <param name="socket">Socket to use (must be connected)</param>
        ...
        /// <summary>
        ///     Initiate a new write operation. MUST be done from the thread that holds _writerIsActive (i.e. the background async
        ///     thread or one of the enquers)
        /// </summary>
        ...
        private void OnReadCompleted(object sender, SocketAsyncEventArgs e)
        {
            if (e.BytesTransferred == 0 || e.SocketError != SocketError.Success)
            {
                ...
                if (ChannelFailed != null)
                    ChannelFailed(this, new ChannelFailureEventArgs(this, e.SocketError));
                return;
            }
        ...
        private readonly ConcurrentQueue<object> _outboundQueue = new ConcurrentQueue<object>();
        private readonly WriterContext _writerContext = new WriterContext();
        private readonly WriteCompletedCallbackContext _writeCompletedContext = new WriteCompletedCallbackContext();
